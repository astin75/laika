...
export default function Objects({ currentImageInfo }) {
  ...
  const setVisible = (idx) => {
    ...
    if (newAnnot.regions.rect) newAnnot.regions.rect.visible = true;
    if (newAnnot.regions.polygon) newAnnot.regions.polygon.visible = true;
    if (newAnnot.regions.keypoint) newAnnot.regions.keypoint.visible = true;
    annotationDispatcher?.edit(idx, newAnnot, true);
  };

  const setInvisible = (idx) => {
    const newAnnot = _.cloneDeep(annotations[idx]);
    if (newAnnot.regions.rect) newAnnot.regions.rect.visible = false;
    if (newAnnot.regions.polygon) newAnnot.regions.polygon.visible = false;
    if (newAnnot.regions.keypoint) newAnnot.regions.keypoint.visible = false;
    annotationDispatcher?.edit(idx, newAnnot, true);
  };
